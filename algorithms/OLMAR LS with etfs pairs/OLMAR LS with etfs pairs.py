...
def initialize(context):
    ...
    context.stocks = [sid(8554), sid(41382), sid(41959), sid(19658), symbol('QQQ'), sid(52546)]
                      # [symbol('SPY'),symbol('SPLV',symbol('XSW'),symbol('XLK'),symbol('QQQ'),symbol('ESPO')] 
...
def trade(context,data):
    ...
    for n in range(context.r1,context.r2):
        (weight_l,weighted_port_l, weight_s,weighted_port_s) = get_allocation(context,data,prices.tail(n*390))
    ...
    allocation_optimum = allocation_optimum_l 
    # - allocation_optimum_s 
...
def get_allocation(context,data,prices):
    ...
    for i, stock in enumerate(context.stocks):
        if context.portfolio.positions[stock].amount >0:
            b_t_long[i] = context.portfolio.positions[stock].amount*data.current(stock, 'close')
        else:
            b_t_short[i] = context.portfolio.positions[stock].amount*data.current(stock, 'close')
