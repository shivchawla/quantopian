...
def make_pipeline():
    ...
    f21 = FCFStd(mask = base_universe)
    f22 = peg_ratio_std(mask = base_universe)
    f23 = SalesGrowthStability(mask = base_universe)
    f24 = fcf_growth_mean(mask = base_universe)
    f25 = Altman_Z_Stability(mask = base_universe)
    f26 = Current_Ratio_Std(mask = base_universe)
...
'fqn8_sales_yield_cons']].std(axis=1)
    ...
    alpha40 = alpha40.sub(alpha40.mean())
    
    # alpha_c1 = (alpha1_c + alpha2_c + alpha3_c + alpha38 + alpha39 + alpha34 + alpha36)/8 
    alpha_c1 =  (alpha1_c + alpha2_c + alpha3_c + alpha38)/4
    
    alpha_c2 = (output['f16'] + output['f17'] + output['f18'] + output['f19'] + output['f20'] + output['f21'] + output['f22'] + output['f23'] + output['f24'] + output['f25'] + output['f26'])/11
    ...
    output['signal_rank'] = alpha_c #ra/nAlphaFactors
...
def optimize(context, alpha, factor_loadings, beta, cov_style, cov_sector, var, adv_wt, try_opt):
    ...
    # - gamma_port_deviation*port_deviation)
