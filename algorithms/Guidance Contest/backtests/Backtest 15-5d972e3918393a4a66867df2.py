...
def make_pipeline():
    ...
    fq0_mcap = Previous([cp], window_length=1)
    fqn1_mcap = Previous([cp], window_length=1*66)
    fqn2_mcap = Previous([cp], window_length=2*66)
    fqn3_mcap = Previous([cp], window_length=3*66)
    fqn4_mcap = Previous([cp], window_length=4*66)
    fqn5_mcap = Previous([cp], window_length=5*66)
    fqn6_mcap = Previous([cp], window_length=6*66)
    fqn7_mcap = Previous([cp], window_length=7*66)
    fqn8_mcap = Previous([cp], window_length=8*66)
...
def before_trading_start(context, data):
    ...
    output['guid_sales_to_mcap_q1'] = output['fq1_sales_guid']/output['fq0_mcap']
    output['guid_sales_to_mcap_q0'] = output['fq0_sales_guid']/output['fqn1_mcap']
    output['guid_sales_to_mcap_qn1'] = output['fqn1_sales_guid']/output['fqn2_mcap']
    output['guid_sales_to_mcap_qn2'] = output['fqn2_sales_guid']/output['fqn3_mcap']
    output['guid_sales_to_mcap_qn3'] = output['fqn3_sales_guid']/output['fqn4_mcap']
    output['guid_sales_to_mcap_qn4'] = output['fqn4_sales_guid']/output['fqn5_mcap']
    output['guid_sales_to_mcap_qn5'] = output['fqn5_sales_guid']/output['fqn6_mcap']
    output['guid_sales_to_mcap_qn6'] = output['fqn6_sales_guid']/output['fqn7_mcap']
    output['guid_sales_to_mcap_qn7'] = output['fqn7_sales_guid']/output['fqn8_mcap']
...
'guid_sales_to_mcap_qn6', 'guid_sales_to_mcap_qn7']]
    ...
    output['i'] = 0
    output.loc[alpha4.notnull(), 'i'] = 1.0
    alpha4_mcap_m = (alpha4*output['mkt_cap'])/(output['i']*output['mkt_cap']).sum()

    alpha1 = alpha1.sub(alpha1.mean())
    alpha2 = alpha2.sub(alpha2.mean())
    alpha3 = alpha3.sub(alpha3.mean())
    alpha4 = alpha4.sub(alpha4_mcap_m)
    
    output['signal_rank'] = (alpha1+alpha2+alpha3+alpha4)/4
