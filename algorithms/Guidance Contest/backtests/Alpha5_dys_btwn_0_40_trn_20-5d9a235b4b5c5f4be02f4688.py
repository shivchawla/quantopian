...
def before_trading_start(context, data):
    ...
    alpha2_g = signalize(z21*output['flag']) 
    ...
    output['flag3'] = 1.0 
    output.loc[output['guidance_days_prev'] < 5, 'flag3'] = 0.0    
    output.loc[output['guidance_days_prev'] > 40, 'flag3'] = 0.0
    
    
    alpha3_gc = signalize(-z3*output['flag3']) 
...
'guid_sales_to_mcap_qn6', 'guid_sales_to_mcap_qn7']]).mean(axis=1)
    ...
    output['signal_rank'] = alpha5_g
    # alpha1_g + alpha2_g + alpha4_g + (alpha3_gc + alpha5_g + alpha6_ga)/3
...
def record_vars(context, data):
    ...
    # record("1-4", context.alpha1.corr(context.alpha4))
    # record("2-4", context.alpha2.corr(context.alpha4))
    # record("3-4", context.alpha3.corr(context.alpha4))
    record("5-3", context.alpha5.corr(context.alpha3))
    # record("6-4", context.alpha6.corr(context.alpha4))
