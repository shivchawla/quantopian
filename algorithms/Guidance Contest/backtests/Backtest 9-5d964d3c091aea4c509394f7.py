...
def before_trading_start(context, data):
    ...
    print "Sales Guidance Names"
    print output[['fq1_sales_guid',
            'fq0_sales_guid',
            'fqn1_sales_guid',
            'fqn2_sales_guid',
            'fqn3_sales_guid',
            ...
            'fqn5_sales_guid',
            'fqn6_sales_guid',
            'fqn7_sales_guid']].mean(axis=1).count()
    
    
    print "Alpha1"
    print "Non-Null Count"
    print guid_eps_std.count()
    
    context.z1 = guid_eps_std.count()
    # print guid_eps_std
    alpha1 = signalize(-guid_eps_std)
    
    z2 = (output['fq1_sales_guid'] - output['fq0_sales_guid'])/output['fq0_sales_guid']
    print "Alpha2"
    print "Non-Null Count"
    print z2.count()
    context.z2 = z2.count()
    ...
    print "Full Count - Full Signal"
    print emp[['mean']].count()
    context.total = emp[['mean']].count()
    
    alpha3 = signalize(-z3) 

    output['signal_rank'] = (alpha1+alpha3)/2 #(alpha1+alpha2+alpha3)/3
