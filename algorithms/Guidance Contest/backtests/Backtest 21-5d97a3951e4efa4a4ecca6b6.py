...
def initialize(context):
    ...
    context.max_style_exposure = 0.4
...
def make_pipeline():
    ...
    fq0_cp = Previous([cp], window_length=1)
    fqn1_cp = Previous([cp], window_length=1*66)
    fqn2_cp = Previous([cp], window_length=2*66)
    fqn3_cp = Previous([cp], window_length=3*66)
    fqn4_cp = Previous([cp], window_length=4*66)
    fqn5_cp = Previous([cp], window_length=5*66)
    fqn6_cp = Previous([cp], window_length=6*66)
    fqn7_cp = Previous([cp], window_length=7*66)
    fqn8_cp = Previous([cp], window_length=8*66)
...
def before_trading_start(context, data):
    ...
    alpha2 = (signalize(z21) + signalize(z22) + signalize(z23))/3           
    ...
    emp['z2'] = z21
    emp['z3'] = z3
    emp['mean'] = emp[['z1','z2', 'z3']].mean(axis=1)    
    
    context.total = emp[['mean']].count()
    
    alpha3 = signalize(-z3) 
    
    # alpha1_mcap_m = (alpha1*output['mkt_cap'])/(alpha1*output['mkt_cap']).sum()
    # alpha1 = alpha1.sub(alpha1_mcap_m)
    
    # alpha3_mcap_m = (alpha2*output['mkt_cap'])/(alpha2*output['mkt_cap']).sum()
    # alpha3 = alpha2.sub(alpha2_mcap_m)
    ...
    y = output[['guid_sales_to_mcap_q1', 
...
'guid_sales_to_mcap_qn6', 'guid_sales_to_mcap_qn7']]

    alpha4 = signalize(y.mean(axis=1))
    
    
    output['guid_sales_to_cp_q1'] = output['fq0_cp']/output['fq1_sales_guid']
    output['guid_sales_to_cp_q0'] = output['fqn1_cp']/output['fq0_sales_guid']
    output['guid_sales_to_cp_qn1'] = output['fqn2_cp']/output['fqn1_sales_guid']
    output['guid_sales_to_cp_qn2'] = output['fqn3_cp']/output['fqn2_sales_guid']
    output['guid_sales_to_cp_qn3'] = output['fqn4_cp']/output['fqn3_sales_guid']
    output['guid_sales_to_cp_qn4'] = output['fqn5_cp']/output['fqn4_sales_guid']
    output['guid_sales_to_cp_qn5'] = output['fqn6_cp']/output['fqn5_sales_guid']
    output['guid_sales_to_cp_qn6'] = output['fqn7_cp']/output['fqn6_sales_guid']
    output['guid_sales_to_cp_qn7'] = output['fqn8_cp']/output['fqn7_sales_guid']
    
    z = output[['guid_sales_to_cp_q1', 
'guid_sales_to_cp_q0',
'guid_sales_to_cp_qn1',
'guid_sales_to_cp_qn2',
'guid_sales_to_cp_qn3',
'guid_sales_to_cp_qn4',
'guid_sales_to_cp_qn5',
'guid_sales_to_cp_qn6', 'guid_sales_to_cp_qn7']]

    alpha5 = signalize(z.mean(axis=1))
        
    output['signal_rank'] = alpha5 #(alpha1+alpha2+alpha3+alpha4)/4
