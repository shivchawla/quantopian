...
def before_trading_start(context, data):
    ...
    #Signal timing after announcement 
    output['flag'] = 1.0 
    ...
    output.loc[output['guidance_days_prev'] > 70, 'flag'] = -1.0 #70 = 5*14 (14 weeks)
    alpha11_g = signalize(-guid_eps_std*output['flag'])

    
    output['flag'] = 0.0 
    output.loc[output['guidance_days_prev'].isnull(), 'flag'] = 1.0
    output.loc[output['guidance_days_prev'] < 15, 'flag'] = 1.0
    ...
    alpha2_g = signalize(z21) + signalize(z22) + signalize(z23)  
...
'guid_sales_to_mcap_qn6', 'guid_sales_to_mcap_qn7']]).mean(axis=1)
    ...
    output.loc[output['guidance_days_prev'] > 5, 'flag'] = 1.0
    output.loc[output['guidance_days_prev'] < 40, 'flag'] = 1.0
    # alpha4_out1 = signalize(y.mean(axis=1)*output['flag'])
    ...
    output.loc[output['guidance_days_prev'] < 5, 'flag'] = 0.0
    output.loc[output['guidance_days_prev'] > 40, 'flag'] = 0.0
    alpha41_g = signalize(z4*output['flag'])
    ...
    output['signal_rank'] = alpha4_g + alpha41_g
    # alpha1_g + alpha11_g - 25.81
...
def optimize(context, alpha, factor_loadings, beta, cov_style, cov_sector, var, try_opt):
    ...
    turnover = np.linspace(0.2, context.max_turnover, num=100)
