...
def get_allocation(context,data,prices):
    ...
    denom = context.portfolio.portfolio_value
    # test for divide-by-zero case
    if denom > 0:
        b_t = np.divide(b_t,denom)
    else:
        #b_t = np.zeros(len(context.stocks))
        b_t = np.zeros(len(context.stocks)) / len(context.stocks)
    ...
    for i,stock in enumerate(context.stocks):
        price_rel = x_tilde[i]
        context.ls[stock] = 1
        if price_rel < 1:# and price_rel > -0.003:
            price_rel = price_rel - 1
            context.ls[stock] = 0
        elif price_rel > 1:# and price_rel < 0.003:
            price_rel = price_rel - 1
            context.ls[stock] = 0
        #log.info(price_rel)    
        x_tilde[i] = price_rel
    ...
    cons = ({'type': 'ineq', 'fun': lambda x:  np.sum(abs(x)) - 2.0},{'type': 'ineq', 'fun': lambda x:  2.5 - np.sum(abs(x))},{'type': 'ineq', 'fun': lambda x:  np.sum(x) + 0.05}, {'type': 'ineq', 'fun': lambda x:  -np.sum(x) + 0.15},{'type': 'ineq', 'fun': lambda x: np.dot(x,x_tilde) - context.eps})# {'type': 'ineq','fun': lambda x: context.turnover - np.sum(abs(x-b_t))})
   ...
   # res = optimize.minimize(totalreturn, b_t, args=x_tilde, method='SLSQP', constraints=cons, bounds=bnds)
    ...
    res = optimize.minimize(norm_squared, b_t, args=(b_t,x_tilde),method='SLSQP',constraints=cons,bounds=bnds, options={'disp': False,  'maxiter': 1000, 'iprint': 1, 'ftol': 1e-6})
