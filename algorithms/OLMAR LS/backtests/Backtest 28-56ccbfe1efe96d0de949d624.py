...
def get_allocation(context,data,prices):
    ...
    #cons = ({'type': 'ineq', 'fun': lambda x:  np.sum(abs(x)) - 2.0},{'type': 'ineq', 'fun': lambda x:  2.5 - np.sum(abs(x))},{'type': 'ineq', 'fun': lambda x:  np.sum(x) + 1}, {'type': 'ineq', 'fun': lambda x:  -np.sum(x) + 1},{'type': 'ineq', 'fun': lambda x: np.dot(x,x_tilde) - context.eps})# {'type': 'ineq','fun': lambda x: context.turnover - np.sum(abs(x-b_t))})
    
    cons = ({'type': 'ineq', 'fun': lambda x:  np.sum(x) - 1.0},{'type': 'ineq', 'fun': lambda x: np.dot(x,x_tilde) - context.eps})# {'type': 'ineq','fun': lambda x: context.turnover - np.sum(abs(x-b_t))})
