...
def make_pipeline():
    ...
    fs = mean_rev(mask = base_universe)
...
def before_trading_start(context, data):
    ...
    # print(output[['sector', 'weightedFactor3', 'weightedFactor5', 'weightedFactor9']])
    ...
    # output = output.groupby(["sector"]).apply(lambda x: x.sort_values(["vol"], ascending = False)).reset_index(drop=True)
    # select top N rows within each continent

    # output = output.groupby('sector').head(100)
    # t = output.groupby('sector').tail(50)
    
    # output = h.append(t, ignore_index = True).drop_duplicates(subset=['esid']) 
    ...
    # print(net_alpha1)
    # context.alpha = signalize(net_alpha1 + net_alpha2) + signalize(output['fs'])
    # context.alpha = signalize(output['fs'])
    context.alpha = signalize(net_alpha1 + net_alpha2) 
