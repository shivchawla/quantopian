...
def get_allocation(context,data,prices):
    ...
    if denom_long > 0:
        ...
        #b_t_long = np.random.uniform(0,1,len(context.stocks)) #np.ones(len(context.stocks)) / len(context.stocks)
    ...
    if denom_short < 0:
        ...
        #b_t_short = np.random.uniform(0,1,len(context.stocks))#np.ones(len(context.stocks)) / len(context.stocks)
    ...
    #seed portfolio
    #res_long = optimize.minimize(norm_squared, b_0_long, args=(x_tilde_long),method='SLSQP',constraints=cons_long,bounds=bnds_l, options={'disp': False,  'maxiter': 100, 'iprint': 1, 'ftol': 1e-6})
    #res_short = optimize.minimize(norm_squared, b_0_short, args=(x_tilde_short),method='SLSQP',constraints=cons_short,bounds=bnds_s, options={'disp': False,  'maxiter': 100, 'iprint': 1, 'ftol': 1e-6})
