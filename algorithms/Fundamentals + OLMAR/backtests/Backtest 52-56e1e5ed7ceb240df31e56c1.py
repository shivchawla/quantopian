...
def initialize(context):
    ...
    context.indices = symbols('SPY','OIL','GLD','TLT','QQQ')
    context.r1 = 1
    context.r2 = 21
    context.btret = 1
...
def get_allocation(context,data,prices):
    ...
    if denom_long > 0:
        ...
        #b_t_long = np.random.uniform(0,1,len(context.stocks)) #
        #b_t_long = b_t_long/sum(b_t_long)
        
    if denom_short < 0:
        b_t_short = np.divide(b_t_short,denom_short)
    else:     
        b_t_short = np.ones(len(context.stocks)) / len(context.stocks)
        #b_t_short = np.random.uniform(0,1,len(context.stocks))#
        #b_t_short = b_t_short/sum(b_t_short)
        
        
    x_tilde_long = np.zeros(len(context.stocks))
    x_tilde_short = np.zeros(len(context.stocks))
    
    b_0_g = np.ones(len(context.stocks)) / len(context.stocks)
    #b_0 = np.random.uniform(0,1,len(context.stocks))
    #b_0 = b_0/sum(b_0)
    #b_0 = np.zeros(len(context.stocks))
    b_0 = b_0_g
    ...
    res_long = optimize.minimize(norm_squared, b_0_g, args=(b_t_long,x_tilde_long),method='SLSQP',constraints=cons_long,bounds=bnds_l, options={'disp': False,  'maxiter': 100, 'iprint': 1, 'ftol': 1e-6})
   
    res_short = optimize.minimize(norm_squared, b_0_g, args=(b_t_short,x_tilde_short),method='SLSQP',constraints=cons_short,bounds=bnds_s, options={'disp': False,  'maxiter': 100, 'iprint': 1, 'ftol': 1e-6})
    ...
    if res_long.success and (np.dot(allocation_l,x_tilde_long) - context.eps > 0.0) and res_short.success and (np.dot(allocation_s,x_tilde_short) - context.eps) > 0.0:
       ...
       return (np.dot(allocation_l,x_tilde_long), np.dot(allocation_l,x_tilde_long)*allocation_l, context.btret, context.btret* b_t_short, res_long.nit, res_short.nit)
    elif res_short.success and (np.dot(allocation_s,x_tilde_short) - context.eps > 0.0):
       return (context.btret, context.btret*b_t_long, np.dot(allocation_s,x_tilde_short), np.dot(allocation_s,x_tilde_short)*allocation_s, res_long.nit, res_short.nit)
    else:            
        return (context.btret, context.btret*b_t_long, context.btret, context.btret* b_t_short, res_long.nit, res_short.nit)
