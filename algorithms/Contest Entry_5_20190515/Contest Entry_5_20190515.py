...
def before_trading_start(context, data):
    ...
    sector_mkt_cap = (inv_mk_rank*output['mkt_cap']).groupby([output.index.get_level_values(0), output['sector']]).transform('sum')

    output['weightedAlpha1'] = -output['f1']*output['mkt_cap']
    
    _as1 = output['weightedAlpha1'].groupby([output.index.get_level_values(0), output['sector']]).transform('sum')/sector_mkt_cap

    # Alpha Factors
    alpha1 = signalize(transform(output, 'f1', 1)*mk_rank)
    # alpha1_sector = signalize(_as1)
    # gk['alpha'] = signalize(gk['weightedAlpha']/gk['mkt_cap'])
    ...
    ra = alpha1 + alpha2 + alpha3 + 1.5*alpha4 + 0.5*alpha5 + alpha6 + alpha7 + alpha8 + alpha9 + alpha10 + alpha13 + alpha14 + alpha15 + alpha16
