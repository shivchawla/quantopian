...
def optimize(context, alpha, factor_loadings, beta):
    ...
    st_loadings = scipysp.csc_matrix(np.matrix(
                factor_loadings['short_term_reversal'].values.reshape((1, nstocks))))
    
    volatility_loadings = scipysp.csc_matrix(np.matrix(
                factor_loadings['volatility'].values.reshape((1, nstocks))))
    
    momentum_loadings = scipysp.csc_matrix(np.matrix(
                factor_loadings['momentum'].values.reshape((1, nstocks))))
    
    momentum_deviation = 5*cp.sum_squares(momentum_loadings*w - 0.0)
    volatility_deviation = 5*cp.sum_squares(volatility_loadings*w - 0.0)
    st_deviation = 5*cp.sum_squares(st_loadings*w - 0.0)
    total_deviation = momentum_deviation + volatility_deviation + st_deviation + beta_deviation
    
    objective = cp.Maximize(total_alpha - total_deviation)
    ...
    try:
        ...
        context.total_alpha = np.sum(c*w)/100
        context.momentum = momentum_loadings*w
        context.short_term_rev = st_loadings*w
        context.volatility = volatility_loadings*w
...
def record_vars(context, data):
    record(
           ...
           Reversal = context.short_term_rev,
)
