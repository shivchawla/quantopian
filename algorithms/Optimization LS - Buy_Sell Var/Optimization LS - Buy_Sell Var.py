...
def make_pipeline():
    ...
    pipe_screen = base_universe & f1Mask
...
def optimize(context):
    ...
    #    2*nStocks (Phi_B bounds)  
    #    2*nStocks (Phi_S bounds)  
     
    ones_matrix = matrix(1.0, (nstocks, 1))
    zeros_matrix = matrix(0.0, (nstocks, 1))
    ...
    for col_name in context.sectors:
        _loadings = sparse([
                matrix(factor_loadings[col_name].values, (nstocks, 1)),
                -matrix(factor_loadings[col_name].values, (nstocks, 1))]).T
    ...
    # print sector_exp_matrix
    
    style_exp_matrix = None
    nstyles = len(context.styles)
    style_exp_bounds = matrix(context.max_style_exposure, (nstyles,1))
    ...
    current_net_exposure = sum(initial_portfolio)
    max_net_exposure_adj = context.max_net_exposure - current_net_exposure
    min_net_exposure_adj = -context.max_net_exposure - current_net_exposure
    
    net_exp_bounds_u = matrix(max_net_exposure_adj, (1,1))
    net_exp_bounds_l = matrix(-min_net_exposure_adj, (1,1))
    ...
    current_gross_exposure = sum(abs(initial_portfolio))
    max_gross_exposure_adj = context.max_leverage - current_gross_exposure
    min_net_exposure_adj = -context.max_n - current_gross_exposure
   
    gross_exp_bounds_u = matrix(context.max_leverage, (1,1))
    gross_exp_bounds_l = -1*matrix(context.min_leverage, (1,1))
    ...
    turnover_bound = matrix(max_turnover, (1,1))
     
    # Group constraints - 10/11
    # Market beta exposure
    # lBeta < (B1*W1 + B2*W2 + ... + BnWn) < uBeta
    market_beta_exp_matrix = sparse([
            matrix(beta.values, (nstocks,1)),
            -matrix(beta.values, (nstocks,1))]).T
    
    market_beta_exp_bound = matrix(context.max_beta, (1,1))
    ...
    h = matrix([
            ...
            zeros_matrix, # Phi_B > 0
            matrix(context.max_pos_size, (nstocks, 1)), #Phi_B < U
            
            zeros_matrix, #Phi_S > 0
            matrix(context.max_pos_size, (nstocks, 1)) #Phi_S < U
