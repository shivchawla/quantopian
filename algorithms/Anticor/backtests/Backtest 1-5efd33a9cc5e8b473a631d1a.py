...
def anticor(context, data, b):
    ...
    price_history = data.history(context.etfs, fields="price", bar_count=2*w+1, frequency="1d")
...
def rebalance(context, data):
    ...
    for (k,v) in wtdict.items():
        try:
            idx = context.etf_symbols.index(k)
            order_target_percent(context.etfs[idx], v)
        except:
