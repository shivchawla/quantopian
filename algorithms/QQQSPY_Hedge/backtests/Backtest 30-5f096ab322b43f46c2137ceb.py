...
def closepositions(context,data):
    lev = context.lev/2
    delta = min(context.delta, 0.1 - (2/3)*max(0.02, context.vol)/0.2)
    
    # 0.03/0.2  = 0.1 - (0.1/0.15) * 0.15
    
    order_target_percent(context.QQQ, lev + delta)
    order_target_percent(context.SPY, -lev +delta)
    ...
    # and context.eod == False:
        ...
        # or (context.vol > 0.03 and r > 1.01): 
            ...
            if nOrdersQQQ == 0 and nOrdersSPY == 0:
                # and nOrdersXLK == 0:
                order_target_percent(context.QQQ, lev + context.delta)
                order_target_percent(context.SPY, -lev + context.delta)       
                # order_target_percent(context.XLK, -lev/2)
