...
def initialize(context):
    ...
    context.eod = False
    context.factor = 1
    context.baseLev = 1.75
    context.lev = context.baseLev
...
def before_trading_start(context, data):
    ...
    # context.factor = 1
    oldLev = context.lev
    context.factor = 1 - context.dd*5
    context.lev = context.baseLev * context.factor
    context.intradayLev = 3.8*(1 - 0.3*oldLev)  + context.lev/2
    print("Intraday Lev")
    print(context.intradayLev)
    print("Daily Lev")
    print(context.lev)
...
def rebalance(context, data):
    ...
    lp = hist[context.QQQ].values[0]
    # print(lp)
    # ltp = histm[context.QQQ].values[0]
    # print(data.current(context.QQQ, 'last_traded'))
    ...
    if pct > 0.01:        
        ...
        order_target_percent(context.QQQ, context.intradayLev/2)
        order_target_percent(context.SPY, -context.intradayLev/2)
