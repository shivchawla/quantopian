...
def signalize(df):
   series = df / df.std() 
   return ((series.rank() - 0.5)/series.count()).replace(np.nan,0.5)
...
def before_trading_start(context, data):
    ...
    # alpha6 = signalize(signalize(transform(output, 'f6', -1)*mk)*vl)
    # alpha7 = transform(output, 'f7', -1)
    # alpha9 = transform(output, 'f9', 1)
    # alpha10 = signalize(signalize(transform(output, 'f10', 1)*mk)*vl)
    # alpha11 = transform(output, 'f11', 1)
    # alpha12 = transform(output, 'f12', 1)
    # alpha13 = transform(output, 'f13', 1)
    # alpha14 = signalize(transform(output, 'f14', 1)*mk)
    # alpha15 = transform(output, 'f15', 1)
    ...
    alpha5 = alpha5.sub(0.5)
    # alpha5[alpha5 > alpha5.quantile(.75)] = 0.0001
    # alpha5[alpha5 < alpha5.quantile(.25)] = -0.0001
    ...
    output['signal_rank'] = alpha5
    # alpha1 + alpha2 + alpha3 + alpha4 + alpha5 + alpha6 + alpha7 + alpha9 + alpha10 + alpha11 + alpha12 + alpha13 + alpha14 + alpha15
