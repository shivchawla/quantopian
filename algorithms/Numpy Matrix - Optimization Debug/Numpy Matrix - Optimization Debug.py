...
def optimize(context):
    ...
    turnover_matrix = np.matrix(np.hstack((
            zeros_array,
            zeros_array,
            ones_array)).reshape((1, 3*nstocks)))
  
    turnover_bound = np.asmatrix(np.full((1,1), max_turnover))
    
    turnover_lnk_bounds = np.asmatrix(initial_portfolio.reshape((nstocks,1))) 
    ...
    for i in range(nstocks):
        ...
        var_indicator[i] = 1.0
        _var_link_matrix =  np.matrix(np.hstack((
                var_indicator,
                var_indicator,
                -var_indicator)).reshape((1, 3*nstocks)))
        
        if turnover_lnk_matrix_1 is not None:
            turnover_lnk_matrix_1 = np.concatenate((turnover_lnk_matrix_1, _var_link_matrix))
        else:
    ...
    for i in range(nstocks):
        ...
        if turnover_lnk_matrix_2 is not None:
            turnover_lnk_matrix_2 = np.concatenate((turnover_lnk_matrix_2, _var_link_matrix))
        else:
    ...
    for i in range(nstocks):
        ...
        if turnover_var_matrix is not None:
            turnover_var_matrix = np.concatenate((turnover_var_matrix, _var_matrix))
        else:
    ...
    c = np.matrix(np.vstack((
        alpha.values,
        alpha.values,
        zeros_array)).reshape((3*nstocks,1)))
