...
def generateAlpha2(context):
    ...
    alphas = pd.DataFrame() #creates a new dataframe that's empty

    alphas['alpha1'] = alpha1.sub(alpha1.mean())
    alphas['alpha2'] = alpha2.sub(alpha2.mean())
    
    alpha3 = alpha3.sub(alpha3.mean())
    alpha3[alpha3 < 0] = 0
    alphas['alpha3'] = alpha3
    
    alphas['alpha4'] = alpha4.sub(alpha4.mean())
    alphas['alpha5'] = alpha5.sub(alpha5.mean())
    alphas['alpha6'] = alpha6.sub(alpha6.mean())
    alphas['alpha7'] = alpha7.sub(alpha7.mean())
    alphas['alpha8'] = alpha8.sub(alpha8.mean())
    alphas['alpha9'] = alpha9.sub(alpha9.mean())
    alphas['alpha10'] = alpha10.sub(alpha10.mean())
    alphas['alpha11'] = alpha13.sub(alpha13.mean())
    alphas['alpha12'] = alpha14.sub(alpha14.mean())
    alphas['alpha13'] = alpha15.sub(alpha15.mean())
    
    
    
    
def generateAlpha2_old(context):
    """
    Called every day before market open.
    """
    output = algo.pipeline_output('alpha2_pipeline')
