...
def optimize(context, alpha, factor_loadings, beta, covar, var):
    ...
    gamma.value = 50.0/context.nAlphaFactors
    
    risk = cp.quad_form(f, covar.as_matrix()) 
    # risk_idiosyncratic = 10.0 * cp.quad_form(w, D) 
    
    # risk = risk_systematic
    # + risk_idiosyncratic

    mkt_beta_deviation = cp.sum_squares(market_beta_exp_matrix*w - context.target_mkt_beta)
    
    cost = max(cp.sum_squares(w) - 0.000125, 0)

    beta_deviation = 10*mkt_beta_deviation

    objective = cp.Maximize(total_alpha - beta_deviation - gamma*cost)
    ...
    try:
        ...
        print gamma.value*risk.value
        
        print "Concentration Cost"
        print cost.value                                 
