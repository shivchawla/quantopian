...
def initialize(context):
    ...
    # context.LONG = sid(19920)
    # context.SHORT = sid(32265)
...
def openpositions(context, data):
    ...
    """
    o = data.history([context.LONG], 'open', 1, '1d');
    c = data.history([context.LONG], 'close', 2, '1d');  
    
    # print("Yest")
    # print(c[context.LONG].values[0])
    # print("CP")
    # print(o[context.LONG].values[0])
    
    r = o[context.LONG].values[0]/c[context.LONG].values[0]  - 1
    # print("Last Traded")
    # print(data.current(context.LONG, 'last_traded'))
    print("Close 2 Open")
    print(r)
...
def check_positions_for_loss_or_profit(context, data):
    ...
    ct = len(context.portfolio.positions);
    ...
    for security in context.portfolio.positions:
        ...
        if data.can_trade(security) and not get_open_orders(security):
            ...
            if price <= cost_basis * (1 - loss) and current_position > 0:  
                order_target_percent(security, 0)
                context.loss = context.stop_loss
                # if(ct == 2):
                #     if (security is context.LONG):
                #         order_target_percent(context.SHORT, 1.0)
                #     else:
