...
def closepositions(context,data):
    lev_delta = min(1 - context.vol/0.03, 0)
    lev_long = context.lev/2 + lev_delta
    lev_short = context.lev/2 - lev_delta
    order_target_percent(context.QQQ, lev_long)
    order_target_percent(context.SPY, -lev_short)
...
def handle_data(context, data):
    ...
    # and context.eod == False:
        ...
        # or (context.vol > 0.03 and r > 1.01): 
            ...
            lev_delta = min(1 - context.vol/0.02, 0)
            lev_long = context.lev/2 + lev_delta
            lev_short = context.lev/2 - lev_delta
