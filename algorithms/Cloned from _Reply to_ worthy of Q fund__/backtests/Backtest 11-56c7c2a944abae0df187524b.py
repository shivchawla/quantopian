...
def handle_data(context, data):
    ...
    for stock in context.stocks:
        ...
        if stock in security_lists.leveraged_etf_list: # leveraged ETF?
            context.stocks.remove(stock)
        if stock is sid(36929) or stock is sid(27358):
            context.stocks.remove(stock)
...
def get_weighted_port(data,context,prices):
    ...
    for i,stock in enumerate(context.stocks):
        ...
        x_tilde[i] = (0.0*mean_price/prices[-1,i]) + (1.0*prices[-1,i]/mean_price)
...
def rebalance_portfolio(data, context, desired_port):
    ...
    for i, stock in enumerate(context.stocks):
        ...
        order_target_percent(stock, context.leverag*context.ls[stock]*desired_port[i])
