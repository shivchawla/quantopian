...
def before_trading_start(context, data):
    ...
    output['combined'] = transform(output, 'fcfToEv', 1) + transform(output, 'pcf_ratio', -1)
# + transform(output, 'assets_turnover'))/transform(output, 'pretax_margin') 
...
def rebalance(context, data):
    ...
    for (k,v) in  context.portfolio.positions.items():
        positions_val_dict[k] = v.amount * v.last_sale_price
        
    # print positions_val_dict
        
    initial_positions = pd.DataFrame.from_dict(positions_val_dict, orient = 'index')
    
    # print initial_positions
    
    portfolio_value = context.portfolio.portfolio_value
    initial_portfolio = context.max_weights
    
    # print type(initial_portfolio)
    # print initial_positions.count > 0
    # print initial_portfolio
    # print portfolio_value
    
    if not initial_positions.empty:
        _ip = initial_positions 
        # print initial_portfolio.loc[:,1]
        _ip.loc[:,0] *= -1/portfolio_value 
        _ip = _ip.iloc[:,0]
        cmnIdx = list(set(initial_portfolio.index.tolist()) & set(_ip.index.tolist())) 
        initial_portfolio[cmnIdx] = _ip[cmnIdx].values.tolist()
        
    print "Initial Portfolio"
    print initial_portfolio[initial_portfolio < 0]
    ...
    if not alpha.empty:
        ...
        firs_opt_short_weights = opt.calculate_optimal_portfolio(
            objective,
            [
                constrain_pos_size,
                max_leverage,
        ...
        print "First"
        print firs_opt_short_weights
        
        min_weights[firs_opt_short_weights < -0.0001] = 0
        ...
        # max_weights_long = -min_weights
        # min_weights_long = min_weights
        # min_weights_long[:] = 0
        
        # #########
        # # print firs_opt_short_weights < 0
        ...
        # print "Min"
        # print min_weights_long[firs_opt_short_weights < 0]

        # print "Max"
        # print max_weights_long[firs_opt_short_weights < 0]
        
        # print "Max CZR"
        # print max_weights_long[sid(42461)]
    
        holdings = context.portfolio.positions.keys()
    
        print holdings
        
        toBeLiquidated = [sid for sid in holdings if sid not in firs_opt_short_weights.to_dict().keys()]
    
        print toBeLiquidated
    
        for sid in toBeLiquidated:
            order_target_percent(sid, 0.0)

        print "Final"
        print firs_opt_short_weights
        
        optimal = -firs_opt_short_weights
        for k,v in optimal.to_dict().items():
            order_target_percent(k,v)
        
        # algo.order_optimal_portfolio(
        #     objective=opt.TargetWeights(-firs_opt_short_weights),       
        #     constraints=[
        #         constrain_pos_size,
        #         max_leverage,
        #         # dollar_neutral,
