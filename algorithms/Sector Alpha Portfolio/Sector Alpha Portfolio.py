...
def before_trading_start(context, data):
    ...
    # factors = [15]
    # factor_name = 'alpha15'
    # w_factor_name = 'weightedFactor15'
    ...
    # etfs = symbols('XLB','XLY','XLF','IYR','XLP','XLV','XLU','XLC','XLE','XLI','XLK')
    
    etfs = symbols('VAW', 'VCR', 'VFH', 'VNQ', 'VDC', 'VHT', 'VPU', 'VOX', 'VDE', 'VIS', 'VGT')
    
    sector_code_sid = pd.DataFrame({
        'sector': sector_codes, 'sid': etfs})
    ...
    # df['alpha1'] = signalize(-df['weightedFactor1'])  #-11.39/0
    # df['alpha2'] = signalize(df['weightedFactor2']) #0/0
    
    # df[factor_name] = signalize(df[w_factor_name])
    
    df['alpha3'] = signalize(df['weightedFactor3']) #29.14/0.02
    # data['alpha4'] = signalize(data['weightedFactor4']) #12.29/0
    df['alpha5'] = signalize(df['weightedFactor5']) #23.42/0.03
    # df['alpha6'] = signalize(df['weightedFactor6']) #no data
    # data['alpha7'] = signalize(data['weightedFactor7']) #4.99/0.01
    # data['alpha8'] = signalize(data['weightedFactor8']) # ~0
    df['alpha9'] = signalize(df['weightedFactor9']) #29.83/0.02
    # data['alpha10'] = signalize(data['weightedFactor10']) #0
    # data['alpha11'] = signalize(data['weightedFactor11']) #0
    df['alpha13'] = signalize(df['weightedFactor13']) #10.32/0
    # data['alpha14'] = signalize(data['weightedFactor14']) #0
    df['alpha15'] = signalize(df['weightedFactor15']) #11.59/0.01
    ...
    # df[factor_name] = df[factor_name].sub(df[factor_name].mean())
    
    # net_alpha = (-df['alpha2'] + df['alpha5'] + df['alpha6'] \
# - df['alpha9'] - df['alpha13'] + df['alpha15'])/6

    net_alpha = df['alpha3'] + df['alpha5'] - df['alpha9'] - df['alpha13'] + df['alpha15']
    
    # net_alpha = df[factor_name]
