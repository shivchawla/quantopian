...
def rebalance(context, data):
    ...
    if not alpha.empty:
        ...
        cov = compute_covariance(context, data, validSecurities)
        # Running optimization implementation (testing)         
        my_opt_weights = optimize(context, alpha, factor_loadings, beta, cov, vol)
...
def compute_covariance(context, data, securities):
    ...
    for factor_name in context.styles:
        ...
        factor_loadings.loc[-100:, factor_name] = 1.0
        factor_loadings.loc[:100, factor_name] = -1.0
...
def optimize(context, alpha, factor_loadings, beta, cov, vol):
    ...
    # empty_factor_mat = scipysp.csc_matrix(2*nFactors, 2*nFactors) 
    
    # A_org = scipysp.vstack(sector_exp_matrix, 
    #         style_exp_matrix,
    #         market_beta_exp_matrix)
    
    # A = scipysp.block_diag(A_org, empty_factor_mat)
    ...
    total_alpha = c.T*w
    risk_systematic = cp.quad_form(f, cov.as_matrix()) 
    ...
    objective = cp.Maximize(total_alpha - risk_systematic - risk_idiosyncratic - beta_deviation)
