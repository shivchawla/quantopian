...
def before_trading_start(context, data):
    ...
    # alpha16 = alpha15.sub(alpha16.mean())
    
   
    s1 = 1.5*alpha4 + 0.5*alpha5 + alpha6 + alpha7 + alpha8 
    ra = alpha1 + alpha2 + alpha3 + s1 + alpha9 + alpha10 + alpha13 + alpha14 + alpha15 + alpha11 + alpha12
...
def rebalance(context, data):
    ...
    if not alpha.empty:
        ...
        my_opt_weights = optimize(context, alpha, factor_loadings, beta, cov)
...
def optimize(context, alpha, factor_loadings, beta, cov):
    ...
    total_alpha = c.T*w
    risk = cp.quad_form(f, cov.as_matrix()) 
