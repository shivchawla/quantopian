...
def compute_covariance(context, data, securities):
    ...
    # factor_loadings['size'] = -1*factor_loadings['size'] 
    # factor_loadings = factor_loadings.sort_values(by = 'size', ascending=False)
    
    for sector_name in context.sectors:    
        factor_loadings = factor_loadings.sort_values(by = sector_name, ascending=True)
        factor_loadings.loc[:100, sector_name] = 1.0
        factor_loadings.loc[100:, sector_name] = 0.0
        # print sector_name
        # print factor_loadings[sector_name]
        
    # print "Size Loadings"
    # print factor_loadings['size']
    ...
    factor_loadings[np.abs(factor_loadings) != 1.0] = 0.0
    
    
    # factor_loadings[factor_loadings == 1.0] = 0.005
    # factor_loadings[factor_loadings == -1.0] = -0.005
    
    # print "Value Loadings"
    # print factor_loadings['value']
    
    factor_returns = pct_change.dot(factor_loadings)/100
    
    # print "Factor Returns"
    # print factor_returns
    
    cov = factor_returns.cov(min_periods=63)*252
...
def optimize(context, alpha, factor_loadings, beta, covar, var):
    ...
    total_alpha = c.T*w
    risk_systematic = cp.quad_form(f, covar.as_matrix()) 
    risk_idiosyncratic = cp.quad_form(w, D) 
    
    gamma = 1.0
    risk = gamma*(risk_systematic)
    # + risk_idiosyncratic

    mkt_beta_deviation = cp.sum_squares(market_beta_exp_matrix*w - context.target_mkt_beta)
    
    # cost = cp.sum_squares(I*w - 0.01)
    ...
    # + sml_beta_deviation +     vmg_beta_deviation)
    # objective = cp.Maximize(total_alpha - beta_deviation)
    objective = cp.Maximize(total_alpha - risk - beta_deviation)
    ...
    try:
        ...
        print risk_systematic.value
        
        print "Total Idiosyncratic Risk"
        print risk_idiosyncratic.value
