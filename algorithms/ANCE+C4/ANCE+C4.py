...
def frac_rank(a):
    b = (rankdata(a).astype(int) - 0.5)/len(a)
    return np.where(b==np.nan, 0.5, b) 


def make_factors():
    
    class F1_C4(CustomFactor):   
        inputs = [Fundamentals.pcf_ratio, MarketCap(window_length=1)] 
        window_length = 2
        window_safe = True
        # Compute factor1 value
        def compute(self, today, assets, out, var, mcap):
            out[:] = preprocess(frac_rank(var[-2]) * frac_rank(mcap[-1]))

    class F2_C4(CustomFactor):   
        # Pre-declare inputs and window_length
        inputs = [Fundamentals.ps_ratio, MarketCap(window_length=1) , Volatility(window_length=1)] 
        window_length = 2
        window_safe = True
        # Compute factor2 value
        def compute(self, today, assets, out, var, mcap, vol):
            out[:] = preprocess(frac_rank(frac_rank(var[-2]) * frac_rank(mcap[-1]))*frac_rank(vol[-1]))
       
    class F3_C4(CustomFactor):
        # Pre-declare inputs and window_length
        ...
        def compute(self, today, assets, out, ev, var, close, shares, ta):
            out[:] = preprocess(frac_rank(var[-2]/(ev[-2]*close[-2]*shares[-2]*ta[-2])**(1./3.)))

    class F4_C4(CustomFactor):
        # Pre-declare inputs and window_length
        inputs = [Fundamentals.enterprise_value, Fundamentals.free_cash_flow, USEquityPricing.close, Fundamentals.shares_outstanding, Fundamentals.total_assets] 
        window_length = 2
        window_safe = True
        ...
        def compute(self, today, assets, out, ev, var, close, shares, ta):
            out[:] = preprocess(frac_rank(ta[-2]/(ev[-2]*close[-2]*shares[-2])**(1./2.)))
    ...
    factors = [F1,F2,F3,F4,F5,F6,F7,F8,F9,F10,F11,F12,F13,F14,F3_C4]
