...
def make_factors():
    ...
    #     # Pre-declare inputs and window_length
    #     inputs = [Fundamentals.ps_ratio, USEquityPricing.close, Fundamentals.shares_outstanding, ] 
    #     window_length = 2
    #     # Compute factor2 value
    #     def compute(self, today, assets, out, var, close, shares, ):
    #         mcap = close[-1] * shares[-1]
    #         out[:] = preprocess(frac_rank(frac_rank(var[-2]) * frac_rank(mcap))*frac_rank()
    ...
    factors = [F1,F2,F3,F4,F5,F6,F7,F8,F9,F10,F11,F12,F13,F14,F1_C4]
