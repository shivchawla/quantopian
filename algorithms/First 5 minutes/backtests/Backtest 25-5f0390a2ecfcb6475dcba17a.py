...
def rebalance(context, data):
    ...
    order_target_percent(context.SPY3x, 0)

    # print(pct)
    l = 1.0
    if pct > 0.01:        
        order_target_percent(context.QQQ3x, 1)
    else:
        order_target_percent(context.QQQ3x, 0)
        # context.allowed=False
...
def handle_data(context, data):
    ...
    if len(positions) > 0 and context.allowed == True:
        ...
        # or (r > 1.06):
            ...
            order_target_percent(context.QQQ3x, 0)
            # order_target_percent(context.SHORT_SPY, 0.5)
            # order_target_percent(context.LONG, 1)
            context.allowed = False