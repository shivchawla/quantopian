...
def make_pipeline():
    ...
    f1 = AlphaFactor(mask = base_universe)
    f1Mask = f1.top(400)
    f1 = f1.zscore()
...
def optimize(context):
    ...
    # duplicateColumnNames = getDuplicateColumns(factor_loadings)
 
    # print('Duplicate Columns are as follows')
    # for col in duplicateColumnNames:
    #     print('Column name : ', col)

    # duplicateRowsDF = factor_loadings[factor_loadings.duplicated()]
    ...
    sector_exp_bounds = matrix(context.max_sector_exposure, (nsectors,1))
    ...
    net_exp_bounds = matrix(context.max_net_exposure, (1,1))
    ...
    turnover_lnk_bounds = matrix(initial_portfolio, (nstocks,1)) 
    
    # d = |wl + ws - w0|
    # d > wl + ws - w0  => wl + ws - d < w0
    # d > -wl -ws + w0 => -wl -ws -d < -w0
    # Group Constraints - 9 (turnover link vars)
    turnover_lnk_matrix_1 = None
    for i in range(nstocks):
        ...
        if turnover_lnk_matrix_1 is not None:
    ...
    for i in range(nstocks):
        ...
        if turnover_lnk_matrix_2 is not None:
    ...
    for i in range(nstocks):
        ...
        _var_matrix =  sparse([
                ...
                zeros_matrix,
                matrix(var_indicator, (nstocks,1))]).T
        if turnover_var_matrix is not None:
    ...
    h = matrix([
            ...
            zeros_matrix, # xL > 0
            matrix(context.max_pos_size, (nstocks, 1)), #xL < U
            
            matrix(context.max_pos_size, (nstocks, 1)), #-xS < U
            zeros_matrix, #xS < 0
