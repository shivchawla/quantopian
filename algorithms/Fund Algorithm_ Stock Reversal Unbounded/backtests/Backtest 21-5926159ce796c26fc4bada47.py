...
def make_pipeline():
    ...
    universe = (Q1500US() & not_announced_acq_target)
...
def trade(context, data):
    ...
    diff = prices_eod.apply(np.log, axis=1).diff()
    sdev = diff.std()*diff.mean()
    ...
    context.stocks = context.stocks[1:40]
    
    for stock in allstocks:
        if len(context.stocks) < 80:
            context.stocks.append(stock)
