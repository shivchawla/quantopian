...
def before_trading_start(context, data):
    ...
    for f in factors:
        ...
        if stock_port is None:
            stock_port = compute_optimal(context, try_opt)
        else:
            stock_port += compute_optimal(context, try_opt)
    
    # stock_alpha = signalize(output['fs1'] + output['fs2'] + output['fs3'] + output['fs4'] + output['fs5'] + output['fs7'] + output['fs8'])
    context.alpha = signalize(stock_port)

    
    
def compute_optimal(context, try_opt):    
...
def rebalance(context, data):
    ...
    if not alpha.empty:
        ...
        algo.order_optimal_portfolio(
            opt.TargetWeights(compute_optimal(context, try_opt)),
            constraints = []
        )
