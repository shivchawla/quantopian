...
def before_trading_start(context, data):
    ...
    factors = [1,2,3,4,5,7,8]
    
    risk_factor_betas = algo.pipeline_output(
      'risk_pipe'
    ).dropna()
   
    vol_factor_loadings = risk_factor_betas['volatility']
    # print(vol_factor_loadings)
    ...
    for f in factors:
        ...
        vol_risk_stock = (try_opt*signalize(vol_factor_loadings)*try_opt)
        # vol_risk_stock = (try_opt*vol_factor_loadings)
        vol_risk = vol_risk_stock.sum()
        print(fname) 
        print(vol_risk)
        # print(1/pow(2, vol_risk))
        
        if stock_alpha is None:
            # stock_alpha = (1/pow(2,vol_risk))*alpha
            stock_alpha = (1/vol_risk)*alpha
        else:
            # stock_alpha += (1/pow(2,vol_risk))*alpha
            stock_alpha += (1/vol_risk)*alpha
    ...
    context.alpha = signalize(stock_alpha)
