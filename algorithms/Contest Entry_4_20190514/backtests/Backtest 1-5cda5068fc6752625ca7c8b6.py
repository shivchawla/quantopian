...
def signalize(df):
   return ((df.rank() - 0.5)/df.count()).replace(np.nan,0.5)
    ...
    # df = df.clip(df.quantile(0.01), df.quantile(0.99))
...
def initialize(context):
    ...
    algo.attach_pipeline(volume_pipe, 'volume_pipe')
...
def before_trading_start(context, data):
    ...
    v_output = algo.pipeline_output('volume_pipe')
    
    volume = v_output['dollar_volume']
    ...
    alpha1 = signalize(transform(output, 'f1', 1)*mkt_cap)
    alpha2 = signalize(signalize(transform(output, 'f2', 1)*mk_rank)*vol_rank)
    alpha3 = signalize(transform(output, 'f3', 1)*volatility)
    alpha4 = transform(output, 'f4', -1)
    # alpha4 = signalize(transform(output, 'f4', -1)*(1/volatility))
    # alpha5 = transform(output, 'f5', -1)
    alpha5 = signalize(transform(output, 'f5', -1)*volatility)
    alpha6 = transform(output, 'f6', -1)
    # alpha6 = signalize(transform(output, 'f6', -1)*(1/volatility))
    # alpha7 = transform(output, 'f7', -1)
    alpha7 = signalize(transform(output, 'f7', -1)*volatility)
    alpha8 = transform(output, 'f8', -1)
    # alpha8 = signalize(transform(output, 'f8', -1)*(1/volatility))
    alpha9 = signalize(transform(output, 'f9', 1)*vol_rank)
    # alpha9 = signalize(transform(output, 'f9', 1)*(1/volatility))
    alpha10 = transform(output, 'f10', 1)
    # alpha10 = signalize(transform(output, 'f10', 1)*(1/volatility))
    
    # alpha11 = signalize(transform(output, 'f11', 1)*(1/volatility))
    # alpha12 = signalize(transform(output, 'f12', 1)*(1/mkt_cap))
    # alpha13 = transform(output, 'f13', 1)
    ...
    ra = alpha1 + alpha2 + alpha3 + alpha4 + alpha5 + alpha6 + alpha7 + alpha8 + alpha9 + alpha10 + alpha13 + alpha14 + alpha15
    
    
    # print alpha1.corr(alpha2)
    # print alpha1.corr(alpha2)
    # print alpha1.corr(alpha2)
    # print alpha1.corr(alpha2)
    # print alpha1.corr(alpha2)
