...
def rebalance(context, data):
    ...
    """
    hist1 = data.history([context.QQQ], fields="price", bar_count=12, frequency="1m")
    
    pct1 = 3*hist1.pct_change().iloc[1:].sum()[context.QQQ]
    
    hist2 = data.history([context.SPY], fields="price", bar_count=12, frequency="1m")
    
    pct2 = 3*hist2.pct_change().iloc[1:].sum()[context.SPY]
    ...
    # if pct1 - pct2 > 0.001 and pct1 > 0.01:
    if pct1 > 0.01:        
        order_target_percent(context.QQQ, context.lev)
        order_target_percent(context.SPY, 0)
    # elif pct2 - pct1 > 0.001 and pct2 > 0.01:        
    #     order_target_percent(context.SPY, context.lev)
