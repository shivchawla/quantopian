...
def csc_indentity_mat(size):
    return scipysp.csc_matrix(np.identity(size))

def csc_empty_mat(rows, cols):
    return scipysp.csc_matrix((rows,cols))
...
def optimize(context, alpha, factor_loadings, beta):
    ...
    ONE_ROW_MAT = scipysp.csc_matrix(
        np.asmatrix(np.full((1, nstocks), 1.0)))
    ZERO_ROW_MAT = scipysp.csc_matrix(
        np.asmatrix(np.full((1, nstocks), 0.0)))
    ...
    sector_exp_matrix = cp.hstack(
                sector_exp_matrix, 
                csc_empty_mat(nsectors, 1*nstocks))
    ...
    style_exp_matrix = cp.hstack(
                        style_exp_matrix,
                        csc_empty_mat(nstyles, 1*nstocks))
    
    # Group Constraints - 2
    # Dollar Neutral or min_net_exp < sum of weights < max_net_exp
    net_exp_matrix = cp.hstack(
                ONE_ROW_MAT, 
                ONE_ROW_MAT, 
                ZERO_ROW_MAT,
                # ZERO_ROW_MAT
    ...
    turnover_lnk_matrix_1 = cp.hstack(
        csc_indentity_mat(nstocks), 
        csc_indentity_mat(nstocks), 
        -1*csc_indentity_mat(nstocks),
       # csc_empty_mat(nstocks,nstocks)
    ) #Empty sparse matrix
    
    # turnover_lnk_matrix_2 = cp.hstack(
    #     csc_indentity_mat(nstocks),
    #     csc_indentity_mat(nstocks),
    #     csc_empty_mat(nstocks,nstocks), #Empty sparse matrix
    #     csc_indentity_mat(nstocks))
        
    turnover_lnk_bounds_1l = np.full((nstocks), 0.0)
    initial_portfolio
    # print turnover_lnk_bounds_1l
    
    turnover_lnk_bounds_1u = np.full((nstocks), 0.0)
    # initial_portfolio
    ...
    market_beta_exp_matrix = cp.hstack(
    ...
    constraints = [
        ...
        tr >= 0,
        cp.norm(w) <= gross_exp_bounds_u,
        cp.norm(w) >= gross_exp_bounds_l,
    ...
    print "Exposure"
    print np.sum(wL)
    print np.sum(np.abs(wL))
    print np.sum(wS)
    print np.sum(np.abs(wS))
    print np.sum(w)
        
    print np.sum(np.abs(w))

    print "Total Trade"
    print np.sum(actualtr)
    print np.sum(tr)
