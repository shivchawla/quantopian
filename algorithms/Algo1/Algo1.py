...
def before_trading_start(context, data):
    ...
    # output['short'] = transform(output, 'fcfToEv') 
    # output['short'] = transform(output, 'pcf_ratio', -1) 
    output['short'] = transform(output, 'assets_turnover')/transform(output, 'pretax_margin') 
    
    alphaShort = transform(output, 'short', 1)

    alphaShort = alphaShort.sub(alphaShort.mean())
    alphaShort[alphaShort > 0] = 0
    
    output['long'] = transform(output, 'mkt_cap')
    alphaLong = transform(output, 'long', 1)
    alphaLong = alphaLong.sub(alphaLong.mean())
    alphaLong.iloc[:1500] = 0
    alphaLong.iloc[-500:] = 1


    
    alpha = alphaLong
    # alpha = signalize(alphaLong + alphaShort)
...
def rebalance(context, data):
    ...
    if not alpha.empty:
        ...
        beta_neutral = opt.FactorExposure(
            beta_pipeline[['beta']],
            min_exposures={'beta': 0.95},
            max_exposures={'beta': 1.05},
        )
