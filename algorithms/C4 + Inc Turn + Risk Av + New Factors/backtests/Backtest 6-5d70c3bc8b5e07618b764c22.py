...
def initialize(context):
    ...
    context.min_beta = 0.05
    # context.max_beta = 1.15
    context.max_net_exposure = 0.01
    # context.max_net_exposure = 1
...
def before_trading_start(context, data):
    ...
    # ra = alpha_momentum + alpha_size + alpha_value + alpha_vol + alpha_str
...
def rebalance(context, data):
    ...
    if not alpha.empty:
        ...
        beta_neutral = opt.FactorExposure(
            beta[['mkt_beta']],
            min_exposures={'mkt_beta': context.min_beta},
...
def optimize(context, alpha, factor_loadings, beta, cov_style, cov_sector, var, adv_wt):
    ...
    constraints = [
        ...
        w >= np.zeros((nstocks,1)), #min_wt.values.reshape((nstocks,)),
        cp.sum_entries(w) <= context.max_net_exposure + 0.01,
        cp.sum_entries(w) >= context.max_net_exposure - 0.01,
