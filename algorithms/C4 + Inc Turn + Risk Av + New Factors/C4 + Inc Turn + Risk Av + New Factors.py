...
def initialize(context):
    ...
    context.target_mkt_beta = 1.0
    context.normalizing_constant = 0.0714 
    context.optimal = None
...
def before_trading_start(context, data):
    ...
    # alpha_style =  signalize(transform(context.risk_factor_betas, 'momentum', 1)*vol_rank)

    # alpha_style = transform(context.risk_factor_betas, 'volatility', 1)
    # alpha_style = alpha_style.sub(alpha_style.mean())
    
    alpha_vol = transform(context.risk_factor_betas, 'volatility', 1)
    # alpha_vol = alpha_vol.sub(alpha_vol.mean())
    
    alpha_momentum = transform(context.risk_factor_betas, 'momentum', 1)
    # alpha_momentum = alpha_momentum.sub(alpha_momentum.mean())

    alpha_size = transform(context.risk_factor_betas, 'size', 1)
    # alpha_size = alpha_size.sub(alpha_size.mean())
    
    alpha_value = transform(context.risk_factor_betas, 'value', 1)
    # alpha_value = alpha_value.sub(alpha_value.mean())

    alpha_str = transform(context.risk_factor_betas, 'short_term_reversal', 1)
    # alpha_str = alpha_str.sub(alpha_str.mean())

    alpha_style = alpha_size*alpha_value
    alpha_style = alpha_style.sub(alpha_style.mean())
    ...
    alphas = pd.DataFrame()
    alphas['alpha1'] = alpha1
    alphas['alpha2'] = alpha2
    alphas['alpha3'] = alpha3
    alphas['alpha4'] = alpha4
    alphas['alpha5'] = alpha5
    alphas['alpha6'] = alpha6
    alphas['alpha7'] = alpha7
    alphas['alpha8'] = alpha8
    alphas['alpha9'] = alpha9
    alphas['alpha10'] = alpha10
    alphas['alpha13'] = alpha13
    alphas['alpha14'] = alpha14
    alphas['alpha15'] = alpha15
    ...
    alpha_f = alphas.dot(pca.components_.transpose())
...
def rebalance(context, data):
    ...
    if not alpha.empty:
        ...
        # my_opt_weights = convert_alpha_to_weights(alpha)
...
def convert_alpha_to_weights(alpha):
    alpha = alpha.sub(alpha.mean())
    alpha = alpha.sort_values()
    alpha.loc[50:-50] = 0.0
    wt = alpha/alpha.abs().sum()
    wt = wt.sort_values()
    print wt[:5]
    print wt[-5:]
    return wt
...
def optimize(context, alpha, factor_loadings, beta, cov_style, cov_sector, var, adv_wt):
    ...
    constraints = [
        ...
        cp.norm(w,1) >= context.min_leverage,
...
def record_vars(context, data):
    ...
    record("Net Exposure", np.sum(context.optimal))
    pass
