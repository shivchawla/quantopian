...
def initialize(context):
    ...
    context.count = 0
    context.alpha = pd.DataFrame();  
    context.c0 = pd.DataFrame()
    context.c1 = pd.DataFrame()
    context.c2 = pd.DataFrame()
    context.c3 = pd.DataFrame()
    context.c4 = pd.DataFrame()
...
def before_trading_start(context, data):
    ...
    # output['alpha2'] = signalize(0.6*signalize(a0).add(0.2*signalize(a3), fill_value=0.1).add(0.2*signalize(a5), fill_value=0.1))
    
    # output['alpha2'] = signalize(0.8*signalize(a0).add(0.2*signalize(a5), fill_value=0.1))
    temp_alpha = signalize(a0['alpha2'])
                
    if context.count == 0:
        context.c0 = temp_alpha
        context.alpha = context.c0
        
    elif context.count == 1:
        context.c1 = temp_alpha
        print("c1")
        print(context.c1.size)
        print(context.alpha.size)
        context.alpha = context.alpha.add(context.c1, fill_value=0)
        
    elif context.count == 2:
        context.c2 = temp_alpha
        context.alpha = context.alpha.add(context.c2, fill_value=0)
        print("c2")
        print(context.c2.size)
        print(context.alpha.size)

    
    elif context.count == 3:
        context.c3 = temp_alpha
        context.alpha = context.alpha.add(context.c3, fill_value=0)
        print("c3")
        print(context.c3.size)
        print(context.alpha.size)

    
    elif context.count == 4:
      
        context.c4 = temp_alpha
        context.alpha = context.alpha.add(context.c4, fill_value=0)
        print("c4")
        print(context.c4.size)
        print(context.alpha.size)
        ...
        context.c4 = temp_alpha
        context.alpha = signalize(context.c0.add(context.c1, fill_value=0).add(context.c2, fill_value=0).add(context.c3, fill_value=0).add(context.c4, fill_value=0))
        print(context.alpha.size)
...
def rebalance(context, data):
    ...
    if not alpha.empty:
        ...
        # if context.init or context.account.leverage < 0.9:
        #     try:
