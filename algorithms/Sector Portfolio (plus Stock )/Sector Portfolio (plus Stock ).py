...
def initialize(context):
    ...
    #     sector_name = context.sector_names[s]
    #     context.min_sector_weights[sector_name]=-context.max_sector_pos_size
    #     context.max_sector_weights[sector_name]=context.max_sector_pos_size
...
def before_trading_start(context, data):
    ...
    z = ndf['alpha1'] + ndf['alpha3'] + ndf['alpha11'] + ndf['alpha12'] + ndf['alpha10'] + ndf['alpha16']
    # + ndf['alpha_ret']
    ...
    context.sector_alpha = alpha1_sector 
    ndf['min_weights'] = -context.max_sector_pos_size
    ndf['max_weights'] = context.max_sector_pos_size
    
    context.min_sector_weights = ndf['min_weights']
    context.max_sector_weights = ndf['max_weights']
    context.beta_sector_pipeline = algo.pipeline_output('beta_sector_pipe')
        
    # Stock Alpha/beta/loading 
    context.alpha = signalize(-output['vol'])
...
def get_sector_weights(context, data):
    
    alpha = context.sector_alpha
    beta = context.beta_sector_pipeline[['mkt_beta']]
                 
    min_weights = context.min_sector_weights
    max_weights = context.max_sector_weights
    ...
    if not alpha.empty:
        ...
        q_sector_wts = opt.calculate_optimal_portfolio(objective,[
                constrain_pos_size,
                max_leverage,
                dollar_neutral,
                beta_neutral
        ...
        context.min_sector_weights_f = {}
        context.max_sector_weights_f = {}
        
        for s in context.assets:
            sector_name = context.sector_names[s]
            # print sector_name
            # print q_sector_wts[s] 
            # print type(q_sector_wts[s])
            context.min_sector_weights_f[sector_name] = 0
            context.max_sector_weights_f[sector_name] = 0
            if q_sector_wts[s] < 0:
                context.min_sector_weights_f[sector_name] = q_sector_wts[s]
                context.max_sector_weights_f[sector_name] = 0 #q_sector_wts[s] + 0.01
            elif q_sector_wts[s] == 0:
                context.min_sector_weights_f[sector_name] = 0
                context.max_sector_weights_f[sector_name] = 0
            elif q_sector_wts[s] > 0:
                context.min_sector_weights_f[sector_name] = 0 #q_sector_wts[s] - 0.01
                context.max_sector_weights_f[sector_name] = q_sector_wts[s]
    
    print "Min Weights"
    print context.min_sector_weights_f
    print "Max Weights"
    print context.max_sector_weights_f
...
def rebalance(context, data):
    ...
    if not alpha.empty:
        ...
        min_sector_wts = context.min_sector_weights_f
        max_sector_wts = context.max_sector_weights_f
        factor_risk_constraints = opt.experimental.RiskModelExposure(
            factor_loadings,   
            version=opt.Newest,
            min_basic_materials=min_sector_wts['basic_materials'],                       max_basic_materials=max_sector_wts['basic_materials'],                       min_consumer_cyclical=min_sector_wts['consumer_cyclical'],                   max_consumer_cyclical=max_sector_wts['consumer_cyclical'],
            min_financial_services=min_sector_wts['financial_services'], 
            max_financial_services=max_sector_wts['financial_services'], 
            min_real_estate=min_sector_wts['real_estate'], 
            max_real_estate=max_sector_wts['real_estate'], 
            min_consumer_defensive=min_sector_wts['consumer_defensive'], 
            max_consumer_defensive=max_sector_wts['consumer_defensive'],                 min_health_care=min_sector_wts['health_care'], 
            max_health_care=max_sector_wts['health_care'], 
            min_utilities=min_sector_wts['utilities'], 
            max_utilities=max_sector_wts['utilities'],
         min_communication_services=min_sector_wts['communication_services'], 
         max_communication_services=max_sector_wts['communication_services'], 
            min_energy=min_sector_wts['energy'], 
            max_energy=max_sector_wts['energy'], 
            min_industrials=min_sector_wts['industrials'], 
            max_industrials=max_sector_wts['industrials'], 
            min_technology=min_sector_wts['technology'], 
            max_technology=max_sector_wts['technology'])
