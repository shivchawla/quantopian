...
'fqn8_sales_guid']].std(axis=1)
    ...
    alpha_k6 = output['f12'] + output['f13'] + output['f25'] + output['f44'] + output['f59'] + output['f76']
    ...
    alpha_oth = alpha_4 + alpha_52 + alpha_k6 + alpha_54 + alpha_78 + alpha_s3
    
    # alpha = output['f46']
    # (alpha_est + alpha_oth)/2
    alpha = alpha_oth
...
def rebalance(context, data):
    ...
    if not alpha.empty:
        ...
        print try_opt.head(5)
        print try_opt.tail(5)
        
        # print try_opt.describe()
        k = try_opt.sort_values().values
        
        # print k[1]
        # print k[-1]
        
        # context.max_pos_size = max(-k[0],k[-1])
    
        # print context.max_pos_size
        # print "K"
        # print k

        # try_opt = (try_opt - try_opt.mean())/try_opt.std()
        # try_opt = try_opt - try_opt.mean()
        # print "Head"
        # print try_opt.sort_values().head(1)
        
        # print "Tail"
        # print try_opt.sort_values().tail(1)
...
def optimize(context, alpha, factor_loadings, beta, cov_style, cov_sector, var, adv_wt, try_opt):
    ...
    turnover = np.linspace(0.5, context.max_turnover, num=100)
