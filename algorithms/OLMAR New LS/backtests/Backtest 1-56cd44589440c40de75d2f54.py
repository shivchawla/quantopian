...
def get_allocation(context,data,prices):
   ...
   # res = optimize.minimize(totalreturn, b_t, args=x_tilde, method='SLSQP', constraints=cons, bounds=bnds)
    ...
    res_long = optimize.minimize(norm_squared, b_0, args=(b_t_long,x_tilde_long),method='SLSQP',constraints=cons_long,bounds=bnds, options={'disp': False,  'maxiter': 1000, 'iprint': 1, 'ftol': 1e-6})
   
    res_short = optimize.minimize(norm_squared, b_0, args=(b_t_short,x_tilde_short),method='SLSQP',constraints=cons_short,bounds=bnds, options={'disp': False,  'maxiter': 1000, 'iprint': 1, 'ftol': 1e-6})
