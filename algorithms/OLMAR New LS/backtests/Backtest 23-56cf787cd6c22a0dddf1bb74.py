...
def trade(context,data):
    ...
    price_d = history(252,'1d','price')[context.stocks].as_matrix(context.stocks)
    ...
    for n in range(context.r1,context.r2):
        (weight_l,weighted_port_l, weight_s,weighted_port_s) = get_allocation(context,data,prices.tail(n*390),price_d)
    ...
    record(l = sum(abs(allocation_optimum_l)))
    #log.info(sum(abs(allocation_optimum_l)))
    record(s = sum(abs(allocation_optimum_s)))
...
def get_allocation(context,data,prices,price_d):
    ...
    for i,stock in enumerate(context.stocks):
        ...
        mean_price_d = np.mean(price_d[:,i])
        x_tilde_long[i] = (0.0*mean_price/prices[-1,i]) +(1.0*price_d[-1,i]/mean_price_d) 
        x_tilde_short[i] = (0.0*prices[-1,i]/mean_price)
        
        
    x_tilde_long = (x_tilde_long - np.mean(x_tilde_long) + 1)
    x_tilde_short= 1/ (x_tilde_long - np.mean(x_tilde_long) + 1)
    ...
    #, {'type': 'ineq','fun': lambda x: context.turnover - x-b_t_long}, {'type': 'ineq','fun': lambda x: x-b_t_long + context.turnover})
                 #{'type': 'ineq','fun': lambda x: context.turnover - np.sum(abs(x-b_t_long))})
    cons_short = ({'type': 'eq', 'fun': lambda x:  np.sum(x)-1.0},{'type': 'ineq', 'fun': lambda x: np.dot(x,x_tilde_short) - context.eps})
    #, {'type': 'ineq','fun': lambda x: context.turnover - x-b_t_short}, {'type': 'ineq','fun': lambda x: x-b_t_short + context.turnover})
                  #{'type': 'ineq','fun': lambda x: context.turnover - np.sum(abs(x-b_t_short))})
    
    
   
   # res = optimize.minimize(totalreturn, b_t, args=x_tilde, method='SLSQP', constraints=cons, bounds=bnds)
    
    res_long = optimize.minimize(norm_squared, b_0, args=(b_t_long,x_tilde_long),method='SLSQP',constraints=cons_long,bounds=bnds_l, options={'disp': False,  'maxiter': 1000, 'iprint': 1, 'ftol': 1e-6})
   
    #res_short = optimize.minimize(norm_squared, b_0, args=(b_t_short,x_tilde_short),method='SLSQP',constraints=cons_short,bounds=bnds_s, options={'disp': False,  'maxiter': 1000, 'iprint': 1, 'ftol': 1e-6})
    ...
    if res_long.success and (np.dot(allocation_l,x_tilde_long) - context.eps > 0.0):
        log.info(allocation_l)
        return (np.dot(allocation_l,x_tilde_long), np.dot(allocation_l,x_tilde_long)*allocation_l, 0.1, np.zeros(len(context.stocks)))
    else:
        return (0.1, 0.1*b_t_long,0.1, np.zeros(len(context.stocks)))
    ...
    if res_long.success and (np.dot(allocation_l,x_tilde_long) - context.eps > 0.0) and res_short.success and (np.dot(allocation_s,x_tilde_short) - context.eps) > 0.0:
        return (np.dot(allocation_l,x_tilde_long), np.dot(allocation_l,x_tilde_long)*allocation_l, np.dot(allocation_s,x_tilde_short), np.dot(allocation_s,x_tilde_short)*allocation_s)
    elif res_long.success and (np.dot(allocation_l,x_tilde_long) - context.eps > 0.0):
       return (np.dot(allocation_l,x_tilde_long), np.dot(allocation_l,x_tilde_long)*allocation_l, 0.1, 0.1* b_t_short)
    elif res_short.success and (np.dot(allocation_s,x_tilde_short) - context.eps > 0.0):
       return (0.1, 0.1*b_t_long, np.dot(allocation_s,x_tilde_short), np.dot(allocation_s,x_tilde_short)*allocation_s)
    else:            
        return (0.1, 0.1*b_t_long, 0.1, 0.1* b_t_short)
