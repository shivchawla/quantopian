...
def before_trading_start(context, data):
    ...
    ra1 = alpha1 + alpha2 + alpha4 + alpha5 + alpha14
    ra2 = alpha3 + alpha6 + alpha7 + alpha8 + alpha9 + alpha10 + alpha13 + alpha15 + alpha16
    
    # + alpha2 + alpha3 + alpha4 + alpha9 + alpha10 + alpha13 + alpha14 + alpha15 + alpha16
        # alpha5 + alpha6 + alpha7 + alpha8 +
    
    # print "Correlation"
    # print ra.corr(alpha17)
    
    output['signal_rank'] = (ra1 + ra2)
    context.alpha = output['signal_rank']
    
    output['min_weights'] = 0#-context.max_pos_size
...
def optimize(context, alpha, factor_loadings, beta):
    ...
    # # Don't cross zero
    # donot_cross_zero_long = ini_port > 0
    # donot_cross_zero_short = ini_port < 0
    # min_weights.loc[donot_cross_zero_long] = 0
    # max_weights.loc[donot_cross_zero_short] = 0
    
    # max_pos_extn = 0.0095   
    # pos_small = 0.0005       #5,000
    # max_pos_size_with_drift = 0.0125
    # max_pos_size_after_reset = 0.009
    
    # # Don't allow more position (extend) beyond 0.9%
    # max_weights.loc[ini_port >= max_pos_extn] = ini_port.loc[ini_port >= max_pos_extn]
    # min_weights.loc[ini_port <= -max_pos_extn] = ini_port.loc[ini_port <= -max_pos_extn]

    # # Square-off very small positions
    # very_small_long_pos = (ini_port <= pos_small) & (ini_port > 0)
    # very_small_short_pos = (ini_port >= -pos_small) & (ini_port < 0)
    
    # max_weights.loc[very_small_long_pos] = 0
    # min_weights.loc[very_small_short_pos] = 0
    
    # # Position beyond max + 0.0025 are sold to 0.75%
    # very_big_long_pos = (ini_port >= max_pos_size_with_drift) & (ini_port > 0)
    
    # very_big_short_pos = (ini_port <= -max_pos_size_with_drift) & (ini_port > 0)
    
    # max_weights.loc[very_big_long_pos] = max_pos_size_after_reset
    # min_weights.loc[very_big_short_pos] = -max_pos_size_after_reset
    
    # print "Min Weights Largest" 
    # print min_weights.nlargest(10)
    # print "Min Weights Smallest" 
    # print min_weights.nsmallest(10)
    
    # print "Max Weights Largest" 
    # print max_weights.nlargest(10)
    # print "Max Weights Smallest" 
    # print max_weights.nsmallest(10)
    ...
    sector_exp_bounds_u = np.full((nsectors), context.max_sector_exposure)
    sector_exp_bounds_l = np.full((nsectors), 0)
    ...
    style_exp_bounds_u = np.full((nstyles), context.max_style_exposure)
    style_exp_bounds_l = np.full((nstyles), 0.0)
    ...
    market_beta_exp_bound_u = np.full((1), 1.1)
    sml_beta_exp_bound_u = np.full((1), 0.15)
    vmg_beta_exp_bound_u = np.full((1), 0.15)
    
    market_beta_exp_bound_l = np.full((1), 0.9)
    sml_beta_exp_bound_l = np.full((1), -0.15)
    vmg_beta_exp_bound_l = np.full((1), -0.15)
    ...
    constraints = [
        ...
        w >= 0, #min_weights.values.reshape((nstocks,)),
        cp.sum_entries(w) <= 1.1, # context.max_net_exposure,
        cp.sum_entries(w) >= 0.9,#-context.max_net_exposure,
        cp.norm(w,1) <= 1.1, #context.max_leverage,
    ...
    beta_deviation = 5*mkt_beta_deviation
...
def rebalance(context, data):
    ...
    if not alpha.empty:
        ...
        print beta['mkt_beta'].to_frame()
        
        beta_neutral = opt.FactorExposure(
            beta['mkt_beta'].to_frame(),
            min_exposures={'mkt_beta': 0.95},
            max_exposures={'mkt_beta': 1.15},
        )
