...
def optimize(context, alpha, factor_loadings, beta, covar, var):
    ...
    for col_name in context.sectors:
        ...
        if sector_exp_matrix is not None:
            print "WTF-11"
            sector_exp_matrix = np.vstack((sector_exp_matrix, _loadings))
    ...
    for col_name in context.styles:
        ...
        if style_exp_matrix is not None:
            style_exp_matrix = np.vstack((style_exp_matrix, _loadings))
        else:
    ...
    print "WTF-4"
    z = cp.vstack(w, fD)
    # f = scipysp.block_diag([fS,fD].toarray())
    # f = scipysp.bmat([[fS,None],[None,fD]].toarray())
    # f = scipysp.csc_matrix(linalg.block_diag([fS.,fD]))
    ...
    print "Cholesky D"
    print chlskyD
    f = scipysp.csc_matrix(linalg.block_diag(F, chlskyD))*z
    
    print "WTF-5"
    I = np.diag(np.ones(nstocks))
    # I = scipysp.identity(nstocks)
    # Q = scipysp.block_diag(*[20*covar.as_matrix(), I])
    # Q = scipysp.bmat([[20*covar.as_matrix(),None],[None,I]].toarray())
    Q = scipysp.csc_matrix(linalg.block_diag(0.05*covar.as_matrix(), I))
    
    print "WTF-6"
    # Combine all Group Restrictons
    A = cp.vstack(
            scipysp.csc_matrix(sector_exp_matrix), 
            scipysp.csc_matrix(style_exp_matrix),
    ...
    gamma_sys.value = 100.0/context.nAlphaFactors
    
    gamma_unsys = cp.Parameter(sign = "positive")
    gamma_unsys.value = 100.0/context.nAlphaFactors

    gamma_beta = cp.Parameter(sign = "positive")
    gamma_beta.value = 140.0/context.nAlphaFactors
    
    # risk_sys = cp.quad_form(f, covar.as_matrix()) 
    risk_sys = cp.quad_form(f, Q) 
    # risk_unsys = cp.quad_form(w, D) 
    risk_unsys = cp.quad_form(fD, I) 
    ...
    objective = cp.Maximize(total_alpha - gamma_sys*risk_sys - gamma_beta*beta_deviation)
