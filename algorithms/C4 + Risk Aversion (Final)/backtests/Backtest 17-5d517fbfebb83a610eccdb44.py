...
def optimize(context, alpha, factor_loadings, beta, covar, var, adv_wt):
    ...
    max_wt_o = pd.Series(np.ones(nstocks)*context.max_pos_size, index = ini_port.index.tolist())
   
    min_wt_o = pd.Series(-np.ones(nstocks)*context.max_pos_size, index = ini_port.index.tolist())
