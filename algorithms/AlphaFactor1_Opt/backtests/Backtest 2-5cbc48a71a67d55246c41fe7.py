...
def make_pipeline():
    ...
    fcfToEv = FcfToEV(mask = base_universe) / vol
...
def before_trading_start(context, data):
    ...
    output['combined'] = transform(output,'w_assets_turnover')/transform(output,'w_pretax_margin')
    
    output.loc[:10,'combined'] = 0.5 # Assign first 10
    
    # output['combined'] = transform(output,'w_momentum', -1)
    output['signal_rank'] = transform(output, 'combined')
