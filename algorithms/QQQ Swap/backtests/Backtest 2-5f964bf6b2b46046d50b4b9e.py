...
def rebalance(context, data):
    ...
    # if abs(ret_1D) < 0.04 and 
    if abs(ret_today) < 0.04:
        context.lev = 3
    else:
        context.lev = 1
...
def handle_data(context, data):
    ...
    if len(positions) == 1 and context.eod == False and context.intraday == False:
        ...
        if (context.lev == 3 and r <= 0.9933 and context.open_ret > 0) or (context.lev == 3 and r <= 0.9866): 
        ...
        elif (context.lev == 3 and r >= 1.01 and context.open_ret < 0) or (context.lev == 1 and r >= 1.0033 and context.open_ret < 0): 
            
            nOrdersQQQ = len(get_open_orders(QQQ))
            context.intraday = True
            if nOrdersQQQ == 0:
                order_target_percent(QQQ, 3)
                
        elif (context.lev == 3 and r <= 0.99 and context.open_ret > 0) or (context.lev == 1 and r <= 0.9966 and context.open_ret > 0): 
