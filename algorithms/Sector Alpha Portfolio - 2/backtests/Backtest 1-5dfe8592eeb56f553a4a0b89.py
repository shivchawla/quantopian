...
def before_trading_start(context, data):
    ...
    factor_names = ['alpha110', 'alpha49', 'alpha311', 'alpha1314', 'alpha814', 'alpha518', 'alpha1315']
    
    # print(output['f41'])

    data['alpha181'] = signalize(data['weightedFactor181']) 
    data['alpha144'] = signalize(data['weightedFactor144'])
    
    data['alpha95'] = signalize(data['weightedFactor95']) 
    data['alpha86'] = signalize(data['weightedFactor86'])

    data['alpha107'] = signalize(data['weightedFactor107'])
    data['alpha157'] = signalize(data['weightedFactor157'])
    
    
    data['alpha110'] = signalize(data['weightedFactor110']) 
    data['alpha49'] = signalize(data['weightedFactor49'])
    
    data['alpha311'] = signalize(data['weightedFactor311']) 
    data['alpha1314'] = signalize(data['weightedFactor1314'])

    data['alpha814'] = signalize(data['weightedFactor814'])
    data['alpha518'] = signalize(data['weightedFactor518'])
    data['alpha1315'] = signalize(data['weightedFactor1315'])
    ...
    net_alpha = data['alpha110'] + data['alpha49'] + data['alpha311'] - data['alpha1314'] + data['alpha814'] + data['alpha518'] + data['alpha1315'] + data['alpha86'] + data['alpha144'] + data['alpha95'] - data['alpha157'] - data['alpha107'] + data['alpha181']
