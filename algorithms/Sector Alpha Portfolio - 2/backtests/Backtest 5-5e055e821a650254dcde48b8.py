...
def before_trading_start(context, data):
    ...
    # X = df[['alpha3', 'alpha5', 'alpha9']].values
    # X = df[['alpha110', 'alpha49', 'alpha311', 'alpha1314', 'alpha814', 'alpha518']].values
    # pca = PCA(n_components=1)
    # pca.fit(X)
    
    # nn = X.dot(pca.components_[0].T)
    # pca_alpha = pd.DataFrame({
    #         'sid': etfs, 'alpha': nn})
    
    # pca_alpha.set_index(['sid'], inplace=True)
    # pca_alpha = pca_alpha['alpha']
    
    # context.alpha = pca_alpha
        
    context.alpha = net_alpha1 + net_alpha2 + net_alpha3
