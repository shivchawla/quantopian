...
def make_pipeline():
    ...
    f16 = MessageVolume(mask = base_universe)
...
def before_trading_start(context, data):
    ...
    mom = transform(output, 'mom', 1)
    
    # Alpha Factors
    alpha1 = signalize(signalize(transform(output, 'f1', 1)*mk_rank)*mom)
    ...
    alpha16 = signalize(transform(output, 'f16', 1)*vol_rank)
