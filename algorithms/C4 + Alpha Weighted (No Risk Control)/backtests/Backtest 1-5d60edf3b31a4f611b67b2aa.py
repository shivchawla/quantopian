...
def rebalance(context, data):
    ...
    if not alpha.empty:
        ...
        my_opt_weights = optimize(context, alpha, factor_loadings, beta, covariance, variance, adv_wt)
...
def optimize(context, alpha, factor_loadings, beta, covar, var, adv_wt):
            
    nstocks = alpha.size
    ini_port = context.initial_portfolio
    
    max_wt = pd.Series(np.ones(nstocks)*context.max_pos_size, index = ini_port.index.tolist())
   
    min_wt = pd.Series(-np.ones(nstocks)*context.max_pos_size, index = ini_port.index.tolist())
    
    # max_wt = pd.concat([max_wt_o, adv_wt], axis=1).min(axis=1)
    # min_wt = pd.concat([min_wt_o, -1*adv_wt], axis=1).max(axis=1)
