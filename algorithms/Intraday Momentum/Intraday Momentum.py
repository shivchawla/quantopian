...
def handle_data(context, data):
    ...
    for sec in context.security_list: 
        if (sec not in positions) & \
            (sec not in context.already_traded) & \
            data.can_trade(sec): # & \
            # context.invested_capital < 2.0
            ...
            if cthReturn > cthMean:
                ...
                order_target_percent(sec, -context.capital_each)
                context.already_traded[sec] = -1.0 
                context.invested_capital += context.capital_each
