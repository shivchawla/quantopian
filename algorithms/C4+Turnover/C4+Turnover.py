...
def rebalance(context, data):
    ...
    # print context.initial_portfolio
    # print "WTF ALpha"
    # print alpha
    # initial_alpha = np.sum(context.initial_portfolio.multiply(alpha, fill_value=0))
    
    initial_alpha = np.sum(context.initial_portfolio * alpha)
    ...
    if not alpha.empty:
        ...
        my_opt = opt.calculate_optimal_portfolio(
                objective, [
                    constrain_pos_size,
                    max_leverage,
                    dollar_neutral,
        ...
        optimal_alpha = np.sum(my_opt * alpha)
        print "Optimal Alpha"
        print optimal_alpha
        
        if optimal_alpha > initial_alpha:
            ...
            algo.order_optimal_portfolio(
                opt.TargetWeights(context.initial_portfolio),
