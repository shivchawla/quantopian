...
def initialize(context):
    ...
    context.portfolio_val_history = None
...
def before_trading_start(context, data):
    ...
    ra = alpha1 + alpha2 + alpha3 + 1.5*alpha4 + 0.5*alpha5 + alpha6 + alpha7 + alpha8 + alpha9 + alpha10 + alpha13 + alpha14 + alpha15
...
def calc_beta(context):
    context.beta_df[ 'pf_chg'] = context.beta_df.pf.pct_change()
    context.beta_df[ 'spy_chg'] = context.beta_df.spy.pct_change()
    
    context.beta_df = context.beta_df.ix[-context.beta_calc_days:]    # trim to one year
    
    if len(context.beta_df) > 5:
        return context.beta_df.pf_chg.cov(context.beta_df.spy_chg) / context.beta_df.spy_chg.var()
    else:
        return context.target_mkt_beta
...
def record_vars(context, data):
    ...
    try: context.beta_df
    except: context.beta_df = pd.DataFrame([], columns=['pf', 'spy'])
    ...
    if context.beta_df.size > 10:
        print "Computing Beta"
        context.target_mkt_beta = calc_beta(context)
        
    print context.target_mkt_beta
